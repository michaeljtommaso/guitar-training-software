// COPY of apps/web/src/perception/vision/fretboard.test.ts (WP-6) — proves
// the copied fretboard.ts still behaves identically to the perception leg's.
...
describe("fret spacing (real, not uniform)", () => {
  ...
  it("uses equal-tempered (compressing) spacing, not uniform", () => {
    ...
    expect(cell1).toBeGreaterThan(cell5);
  });
});
