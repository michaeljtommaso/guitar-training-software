// COPY of apps/web/src/perception/vision/fretboard.ts (WP-6). The fretboard
// grid overlay must use the SAME equal-tempered fret spacing and string
// convention (1 = high e) as the perception leg, or annotated labels won't
// line up with what the model sees at inference time. KEEP IN SYNC by hand —
// apps/web/src/perception/vision/fretboard.ts is the source of truth and is
// owned by WP-3, not this app.
//
...
// The homography (see homography.ts) maps camera image space → this
// normalized space; everything downstream (finger mapping, overlay grid)
